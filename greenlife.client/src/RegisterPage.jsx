...
function RegisterPage() {
    ...
    const validateField = (name, value) => {
        ...
        if (name === 'name') {
            if (!value) error = 'O campo Nome é obrigatório.';
        ...
        if (name === 'email') {
            ...
            if (!value) error = 'O campo Email é obrigatório.';
            else if (!emailRegex.test(value)) error = 'Formato de email inválido.';
        }

        if (name === 'password') {
            if (!value) error = 'O campo Senha é obrigatório.';
            else if (value.length < 6) error = 'A Senha deve ter no mínimo 6 caracteres.';
        }

        if (name === 'confirmPassword') {
            if (!value) error = 'O campo Confirmar Senha é obrigatório.';
                    ...
                    {['name', 'email', 'password', 'confirmPassword'].map((field, index) => (
                        ...
                        >
                            ...
                            <input
                                ...
                                className={`w-full p-3 mt-2 text-gray-700 bg-gray-50 border ${errors[field] ? 'border-red-500' : 'border-gray-300'} rounded-lg focus:outline-none focus:ring-2 ${errors[field] ? 'focus:ring-red-500' : 'focus:ring-green-700'}`}
